...
- 对照原版 Superpowers `d884ae04edebef577e82ff7c4e143debd0bbec99`，恢复主动探索候选方向、
  推荐优先和用户批准三项有效语义，不恢复固定逐问题询问、逐节批准和所有小改强制规格的流程。
- 多个可行方向通常展示 2～4 个，但该范围只是默认呈现宽度而非思考上限；方向更多时允许分组
  或继续展开，只有一条路径成立时必须说明其他主要路线被什么证据排除。
- 只要承重方向仍开放，确认前不得进入计划或实施；清楚的方案选择、“按推荐来”“可以”“ok”
  等短回复可以作为有效批准，方向清楚后不重复询问。

## RED / GREEN

- 先增加候选空间、推荐、确认门、短回复和 `0.1.5` 版本契约，并新增“过早收敛且未等待批准”
  压力场景。旧实现运行得到 32 项中 28 项通过、4 项按预期失败：三处旧版本和一项缺少主动扫描
  候选空间语义；压力场景 JSON 和测试基础设施正常。
- 实现正文和三处版本后首跑得到 31/32，通过项之外仅发现既有短语“实质可行的方案”被改写时
  少了一个“的”；补回兼容契约后恢复 32/32 GREEN。

## 完整验证

- `npm --prefix .\plugins\superpowers-lite test`：32/32 通过，0 失败、0 跳过。
- 显式枚举全部技能运行 `quick_validate.py`：13/13 输出 `Skill is valid!`。
- `validate_plugin.py .\plugins\superpowers-lite`：输出 `Plugin validation passed`。
- `test-runtime-scripts.sh`：12 项污染定位和临时路径安全的真实 Bash 行为检查全部通过。
- `test-package-codex-plugin.sh`：ZIP/tar.gz 白名单、身份、元数据、可执行位、哈希和重复构建通过。
- Codex-only 禁止路径扫描、三处版本一致性检查、组合差异审查和 `git diff --check` 通过。

## 正式归档

- 从干净实现提交 `e8db12ab55d6e5a6b04097952f7207927bfd0e02` 生成
  `superpowers-lite-0.1.5.zip`。
- 根层只包含 `.codex-plugin/`、`assets/`、`skills/`、`README.md` 和 `LICENSE`。
- 归档包含 54 个条目、13 个技能，SHA-256
